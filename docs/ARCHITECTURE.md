...
### Pentest interno
Con el backend completo, se hizo una ronda de pruebas de intrusión reales contra la app corriendo (no solo revisión de código): inyección SQL, bypass de autenticación (tokens malformados, `alg: none`, secrets adivinados), escalación de privilegios, mass assignment, IDOR entre roles (vía RLS), bypass de rate limiting por IP spoofing, headers de seguridad, CORS, payload gigante, y comportamiento de refresh tokens.

**Bloqueado correctamente, sin cambios necesarios:** inyección SQL (TypeORM parametrizado), bypass de auth (verificación de firma JWT correcta), escalación de privilegios (rol forzado a `cliente` en registro público + whitelist de DTOs rechazando campos no declarados), IDOR (RLS oculta con `404`, no `403`, minimizando información al atacante), bypass de rate limiting vía `X-Forwarded-For` (la app no confía en headers de cliente para IP sin `trust proxy` configurado — pendiente configurarlo correctamente cuando se despliegue detrás de un proxy real).

**3 bugs reales encontrados y corregidos en el momento:**
1. **Registro con email duplicado devolvía `500` en vez de `409`.** La verificación de duplicado (`SELECT` por email) quedaba bloqueada por RLS en el registro público (sin contexto de sesión todavía), así que el código nunca veía al usuario existente y el `INSERT` fallaba después con un error crudo de constraint única. No era explotable (la constraint de la DB evitó el duplicado real), pero el manejo de errores estaba mal. Se corrigió seteando `service_auth` también antes de esa verificación, no solo antes del insert final.
2. **Payload gigante (10MB) devolvía `500` en vez de `413`.** Express sí lo rechazaba correctamente (límite default de 100kb), pero `AllExceptionsFilter` no reconocía `PayloadTooLargeError` (no es una `HttpException` de Nest) y lo enmascaraba como error genérico. Se agregó manejo explícito para errores 4xx crudos de middleware.
3. **Refresh token completamente roto** (hallazgo colateral, más severo que los anteriores): nadie podía renovar su sesión, ni con un token válido. Mismo patrón de causa que el bug 1 — `UsuariosService.findOne(payload.sub)` dentro de `AuthService.refresh()` no tenía contexto de sesión (el endpoint no pasa por `JwtAuthGuard`), así que RLS lo bloqueaba siempre, pero el `catch` genérico de `refresh()` enmascaraba el error real como "token inválido". Se agregó `UsuariosService.findOneParaRefresh()`, que setea `service_auth` para este caso específico.

**2 gaps cerrados tras el pentest:**
- **Headers de seguridad HTTP faltantes** (`X-Powered-By: Express` filtraba el stack; sin `X-Content-Type-Options`, `X-Frame-Options`, `Strict-Transport-Security`, `Content-Security-Policy`). Se agregó `helmet()` en `main.ts`. Verificado que no rompe Swagger UI (su HTML no usa scripts inline, todo se sirve same-origin).
- **CORS con wildcard** (`Access-Control-Allow-Origin: *`, cualquier sitio podía llamar a la API desde el navegador). Se restringió a `CORS_ORIGIN` (variable de entorno, uno o varios orígenes separados por coma), default `http://localhost:3000` para desarrollo — actualizar con el dominio real del frontend en producción.

**1 punto anotado, sin resolver todavía (decisión pendiente, no urgente):** el refresh token, una vez válido, se puede reusar indefinidamente hasta su expiración natural (7 días) — no hay rotación ni mecanismo de revocación (logout, blacklist). Si un refresh token se filtra, queda utilizable hasta que expire. Se deja para cuando se defina el flujo de logout del frontend.

### Segunda ronda: auditoría de dependencias y timing attack
A pedido explícito de revisar si quedaba algo más de seguridad pendiente, se hizo una segunda pasada más allá del pentest de endpoints: `npm audit` y un intento real de timing attack contra el login.

**Vulnerabilidad real y explotable, corregida:** el login tenía un **timing side-channel que permite enumerar emails registrados**. Medido con una herramienta externa (no solo sospecha): la diferencia entre "usuario no existe" (~2ms, corta antes de `bcrypt.compare`) y "usuario existe, password incorrecto" (~280-290ms, corre `bcrypt.compare`) era de ~277ms — trivial de detectar con un script simple, sin necesitar herramientas especializadas. Se corrigió haciendo que `AuthService.login` corra `bcrypt.compare()` **siempre**, contra un hash dummy generado una sola vez al iniciar el servicio cuando el usuario no existe — el tiempo de respuesta queda equivalente en ambos casos (confirmado con mediciones limpias post-fix, por debajo del límite de rate limiting para no contaminar la medición con respuestas `429`).

**`npm audit`: de 19 vulnerabilidades a 15, con las 2 más serias resueltas:**
- **`sanitize-html@2.12.1`** (la versión a la que había bajado por el problema de compatibilidad con Jest) tenía **CVEs de XSS moderados** (bypass de esquema `javascript:` en atributos, mutation-XSS vía `</textarea/>`) — justo en la librería que usamos como defensa contra XSS. Con nuestra configuración específica (`allowedTags: []`, no se permite ningún tag) esos bypasses no aplican — se probó explícitamente con los payloads de esas CVEs y todos quedaron neutralizados — pero de todos modos se resolvió el problema de raíz en vez de convivir con una dependencia vulnerable: se volvió a `sanitize-html@2.17.7` (última versión, sin esas CVEs) y se arregló la incompatibilidad con Jest correctamente, configurando `babel-jest` específicamente para transpilar la dependencia ESM-only (`htmlparser2@12`) en tests — la causa raíz real de por qué se había bajado de versión originalmente.
